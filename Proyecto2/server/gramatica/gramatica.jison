...
// jaavascript
    ...
    const { ErrType } = require ('../class/util/errtype')
%}
%lex
...
'nl' { 
    tokens.push(new Tokens('resw_nl', 'reservada', yytext, yylloc.first_line, yylloc.first_column)); 
    return 'resw_nl';  
}
'objeto' { 
...
'->' { 
    tokens.push(new Tokens('tok_asign', 'operador', yytext, yylloc.first_line, yylloc.first_column)); 
    return 'tok_asign';  
...
%{
    // Tipos
        ...
        const { IfStatement } = require ('../class/Instrucciones/IfStatement')
        const { ListDeclaration } = require ('../class/Instrucciones/ListDeclaration')
        const { ModifyList } = require ('../class/Instrucciones/ModifyList')
        const { ForLoop } = require ('../class/Instrucciones/ForLoop')
        const { WhileLoop } = require ('../class/Instrucciones/WhileLoop')
        const { DoWhileLoop } = require ('../class/Instrucciones/DoWhileLoop')
        ...
        const { SwitchStatement } = require ('../class/Instrucciones/SwitchStatement')
        const { Procedimiento } = require ('../class/Instrucciones/Procedimiento')
        const { LlamadaProcedimiento } = require ('../class/Instrucciones/LlamadaProcedimiento')
        const { GuardarObjeto } = require ('../class/Instrucciones/GuardarObjetos')
        // Expresiones
...
IMPRIMIR :
        resw_imprimir EXPR {$$ = new print(@1.first_line, @1.first_column, $2)} |
        resw_imprimir resw_nl EXPR {$$ = new print(@1.first_line, @1.first_column, $3)} ;
CASTEO :
